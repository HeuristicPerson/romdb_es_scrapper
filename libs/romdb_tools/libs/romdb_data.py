...
"""
File to define the classes representing the contents of ROMdb website.
"""
...
class Version(object):
    ...
    def _get_odate(self):
        ...
        """
        o_date = date_operations.unicode_to_datetime(self.u_mdata_date)
        return o_date

    o_mdata_date = property(fget=_get_odate, fset=None)
...
class Game:
    ...
    def from_json(self, pdx_json):
        self.i_nid = int(pdx_json.get('i_nid', 0))
        self.u_title = unicode(pdx_json.get('s_title', u''))
        self.u_years = unicode(pdx_json.get('s_years', u''))
        self.u_synopsis = unicode(pdx_json.get('s_synopsis', u''))
        self.lu_genres = pdx_json.get('as_genres', [])

        for dx_entry in pdx_json['ao_sagas']:
            o_saga = Saga(dx_entry)
            self.lo_sagas.append(o_saga)

        for dx_entry in pdx_json['ao_versions']:
            o_version = Version()
            o_version.from_json(dx_entry)
            self.lo_versions.append(o_version)
    ...
    def _nice_text_medium(self):
        ...
        u_out += _lines_indent(
            u'│  u_synopsis: ',
            string_operators.sentence_split(self.u_synopsis),
            u'│              ')

        if not self.lo_versions:
            u_out += u'│ lu_versions:\n'
        else:
            for i_version, o_version in enumerate(self.lo_versions):
                if i_version == 0:
                    u_out += u'│ lu_versions: [%s/%s] %s \n' % (
                        o_version.u_romset_platform,
                        o_version.u_romset_crc32,
                        o_version.u_romset_title)

                else:
                    u_out += u'│              [%s/%s] %s \n' % (
                        o_version.u_romset_platform.ljust(7, u' '),
                        o_version.u_romset_crc32,
                        o_version.u_romset_title)
