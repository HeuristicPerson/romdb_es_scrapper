...
from libs.romdb_tools import rdb_rom_info
import libs.romdb_tools
...
# Constants
#=======================================================================================================================
u_PRG_NAME = u'ES-ROMDB Scrapper v1.0 - 2019-04-24'
...
def _scrape_file(po_file, pu_platform, po_dat, po_img_dir):
    ...
    if o_romset is not None:
        o_romdb_result = rdb_rom_info.query_romset_by_crc32(pu_platform, o_romset.u_ccrc32)
        ...
        if o_romdb_result is not None:
            ...
            # Getting the date
            #-----------------
            if o_romdb_result.o_mdata_date is not None:
                u_date = o_romdb_result.o_mdata_date.strftime(u'%Y%m%dT%H%M%S')
            else:
                u_date = u''
...
def scrape(pu_rom_path, pu_platform, pu_dat_path, pu_gamelist, pu_img_dir, pb_print=False):
    ...
    if o_gamelist.is_file():
        o_xml = et.parse(o_gamelist.u_path)
    ...
    for i_file, o_file in enumerate(lo_files):
        o_xgame, o_progress = _scrape_file(o_file, pu_platform, o_dat, o_img_dir)
        if o_xgame is not None:
            i_scrapped += 1
            lo_existing = o_xtree.xpath(u'.//path[text()="%s"]' % o_file.u_path)
            if lo_existing:
                # there should be only one existing element but I won't check that
                lo_existing[0] = et.fromstring(et.tostring(o_xgame, encoding='utf-8'))
            else:
                o_xtree.append(o_xgame)
